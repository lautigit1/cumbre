...
import { ArrowRight, Mail, Lock, Eye, EyeOff, User, Sparkles, Shield, CheckCircle2 } from 'lucide-react';
...
export default function RegistroPage() {
  ...
  const manejarRegistro = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const response = await servicios.identidad.registro({
        nombre: form.nombre,
  ...
  const passwordStrength = () => {
    ...
    if (pw.length < 6) return { text: 'Débil', color: '#ff6b6b', percent: 25 };
    if (pw.length < 10) return { text: 'Media', color: '#ffa502', percent: 50 };
    if (!/[A-Z]/.test(pw) || !/[0-9]/.test(pw)) return { text: 'Buena', color: '#ffb142', percent: 75 };
    return { text: 'Excelente', color: '#26de81', percent: 100 };
  };
